...
def get_retail_validation_summary(
    drugs_df: pl.DataFrame,
) -> dict[str, object]:
    """Get summary statistics for retail validation.

    Args:
        drugs_df: DataFrame with retail_confidence column.

    Returns:
        Dictionary with summary statistics.
    """
    if "retail_confidence" not in drugs_df.columns:
        return {
            "validated": False,
            "message": "Retail validation not performed",
        }

    total = drugs_df.height
    high_count = drugs_df.filter(pl.col("retail_confidence") == "High").height
    low_count = drugs_df.filter(pl.col("retail_confidence") == "Low").height
    unknown_count = drugs_df.filter(pl.col("retail_confidence") == "Unknown").height

    high_pct = (high_count / total * 100) if total > 0 else 0
    low_pct = (low_count / total * 100) if total > 0 else 0

    # Calculate average variance for validated drugs
    validated_df = drugs_df.filter(pl.col("retail_variance_pct").is_not_null())
    avg_variance = validated_df["retail_variance_pct"].mean() if validated_df.height > 0 else None
    ...
    return {
        ...
        "avg_variance_pct": round(avg_variance * 100, 1) if avg_variance else None,
    }
